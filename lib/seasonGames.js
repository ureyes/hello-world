if(Meteor.isClient) {
		...
		gamesList: function() {
			...
			return Game.find({id_season:this.seasonId,_id:idFind,status:{$in:[1,0]}},{sort: {date: 1}, transform:function(game) {
				...
				for(var i=0; i<lengthUsers; i++) {
					...
					PLAYERS_LIST[game._id_users[i].id][game._id] = pick;
				}
	...
	Template.gameRowPlayer.helpers({
		playerPick: function() {
			var hidePick = false;
			var pick = "";
			if(PLAYERS_LIST[this.idPlayer] && PLAYERS_LIST[this.idPlayer][this.idGame]) {
				if(PLAYERS_LIST[this.idPlayer][this.idGame]=="@hide@") {
					hidePick=true;
				} else {
					pick = PLAYERS_LIST[this.idPlayer][this.idGame];
				}
			}
			return {'status':hidePick, 'pick':pick};
		}
	});
}
