...
(function(){
	...
	formvalidate.Manager.prototype.addRules = function(fieldId,fieldRules,alertFlags) {
		this.__fieldList.push(new this.__fieldObject(fieldId,fieldRules,alertFlags));
	};
	formvalidate.Manager.prototype.doValidate = function(){
		...
		for(var i=0; i<lengthFieldList; i++) {
			...
			if(!result.status) {
				this.__fieldList[i].__fieldElement.parents(".form-group").addClass("has-error");
				result.errorAction();
				return false;
			}
			this.__fieldList[i].__fieldElement.parents(".form-group").removeClass("has-error");
		}
		return true;
	};
	...
	formvalidate.Manager.prototype.__validateRules = function(field) {
		...
		if (textTypeList.indexOf(this.__elementType)) {
			...
			var alertMsg = ((field.__alertFlags && field.__alertFlags.alertMsg)?field.__alertFlags.alertMsg:"");
			// Empty value
			if (field.__rules.required && val=="") {	
				return {status:false,errorAction:this.__errorActionFunc({type:"empty", alertmsg:alertMsg})};
			}
			// Email format
			if (field.__rules.email && val!="" && !val.match(/.+@.+\..+/)) {
				return {status:false,errorAction:this.__errorActionFunc({type:"emailFormat",alertmsg:alertMsg})};
			}
			// Minimum characters
			if (field.__rules.minlength && val!="" && field.__rules.minlength>val.length) {
				return {status:false,errorAction:this.__errorActionFunc({type:"minlength", minlength:field.__rules.minlength, alertmsg:alertMsg})};
			}
			// Alpha numeric only. No special chars.
			if (field.__rules.alphanumeric && val!="" && !val.match(/[0-9a-zA-Z]+/)) {
				return {status:false,errorAction:this.__errorActionFunc({type:"alphanumeric", alertmsg:alertMsg})};
			}
			// Must match field (ie. password confirmation)
			if (field.__rules.matchfield && val!="") {
				var valOrig = $('#'+field.__rules.matchfield).val();
				if(valOrig!=val) {
					return {status:false,errorAction:this.__errorActionFunc({type:"mismatch",alertmsg:field.__alert, alertmsg:alertMsg})};
				}
			}
			return {status:true};
		}
	...
	formvalidate.Manager.prototype.__fieldObject = function(fieldId,fieldRules,alertFlags) {
		this.__id = fieldId;
